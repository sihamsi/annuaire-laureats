...
import { Users, Filter, Download, Mail, Phone, MapPinned, CheckCircle, Clock, XCircle, Search, Home, Map, BarChart3, Settings } from 'lucide-react';
...
const AnnuairePage = () => {
  ...
  const [laureats] = useState(mockLaureats);
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white flex items-center justify-between px-8 py-4 border-b shadow-sm">
        ...
        <nav className="flex items-center gap-8">
          ...
          <Link to="/annuaire" className="text-gray-700 hover:text-gray-900 flex items-center gap-2 bg-green-100 px-3 py-1 rounded">
            <Users size={18} />
            <span>Annuaire</span>
          </Link>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 pt-24">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-4xl font-serif text-black">Annuaire des Lauréats</h1>
            <p className="text-gray-600 mt-2">Liste complète des membres de l'association</p>
          </div>
        ...
        {showFilters && (
          <div className="bg-white p-6 rounded-lg shadow-md mb-6 border border-gray-200">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-gray-700">Filière</label>
                <select
              ...
              <div>
                <label className="block text-sm font-medium mb-2 text-gray-700">Promotion</label>
                <select
            ...
            <div className="mt-4 pt-4 border-t flex justify-between items-center">
              <div className="text-sm text-gray-600">
                <span className="font-semibold text-green-700 text-lg">{filteredLaureats.length}</span> lauréat(s) trouvé(s)
              </div>
              <div className="flex space-x-2">
                <span className="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-semibold">
                  {filteredLaureats.filter(l => l.statut === 'validé').length} Validés
                </span>
                <span className="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm font-semibold">
                  {filteredLaureats.filter(l => l.statut === 'en_attente').length} En attente
                </span>
              </div>
            </div>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-100 border-b-2 border-gray-200">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Nom & Prénom</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Filière</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Promotion</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Organisme</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Province</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredLaureats.map(laureat => (
                  <tr key={laureat.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-4">
                      <div className="font-semibold text-gray-800">{laureat.prenom} {laureat.nom}</div>
                      <div className="text-sm text-gray-500">{laureat.description}</div>
                    </td>
                    <td className="px-4 py-4 text-sm text-gray-700">{laureat.filiere}</td>
                    <td className="px-4 py-4 text-sm font-semibold text-green-700">{laureat.promotion}</td>
                    <td className="px-4 py-4">
                      <div className="flex items-center space-x-1 text-sm text-gray-700">
                        <Mail className="w-4 h-4 text-gray-400" />
                        <span className="truncate max-w-[180px]">{laureat.email}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center space-x-1 text-sm text-gray-700">
                    ...
                    <td className="px-4 py-4">
                      <div className="text-sm text-gray-700">{laureat.organisme}</div>
                      <div className="text-xs text-gray-500">{laureat.secteur}</div>
                    </td>
                    <td className="px-4 py-4">
                      <div className="flex items-center space-x-1 text-sm text-gray-700">
                    ...
                    <td className="px-4 py-4">
                      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold ${
                        laureat.statut === 'validé' ? 'bg-green-100 text-green-800' :
                        laureat.statut === 'en_attente' ? 'bg-yellow-100 text-yellow-800' :
                        'bg-red-100 text-red-800'
                      }`}>
                        {laureat.statut === 'validé' ? <CheckCircle className="w-3 h-3 mr-1" /> :
                         laureat.statut === 'en_attente' ? <Clock className="w-3 h-3 mr-1" /> :
                         <XCircle className="w-3 h-3 mr-1" />}
                        {laureat.statut === 'validé' ? 'Validé' : 
                         laureat.statut === 'en_attente' ? 'En attente' : 
                         'Rejeté'}
                      </span>
                    </td>
                  </tr>
        ...
        {filteredLaureats.length === 0 && (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-600 mb-2">Aucun lauréat trouvé</h3>
